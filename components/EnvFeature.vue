<template>
    <div class="container">
      ...
      <div class="output-column">
        ...
        <div>
          ...
          <div v-if="invalidInputs.length > 0" class="error-message">
            Invalid inputs: {{ invalidInputs.join(', ') }}
          </div>
  ...
  const invalidInputs = ref([]); // Track invalid inputs
  ...
  const processDeployInput = () => {
    ...
    invalidInputs.value = []; // Reset the invalid inputs
    ...
    deployValues.forEach(value => {
      if (mappings[value]) {
        ...
        invalidInputs.value.push(value); // Add invalid value to the list
      }
    });
